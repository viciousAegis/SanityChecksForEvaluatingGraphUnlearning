...
from .base_unlearn_algo import UnlearnMethod
...
class GIF(UnlearnMethod):
    ...
    def train_test_split(self):
        self.train_indices, self.test_indices = train_test_split(
            ...
            test_size=self.args.gif_test_ratio,
            random_state=100,
        )
    ...
    def get_gradients(self, unlearn_info=None):
        ...
        loss = F.nll_loss(
            out1[self.data.train_mask],
            self.data.y[self.data.train_mask].type(torch.LongTensor),
            reduction="sum",
        )
        loss1 = F.nll_loss(
            out1[mask1], self.data.y[mask1].type(torch.LongTensor), reduction="sum"
        )
        loss2 = F.nll_loss(
            out2[mask2], self.data.y[mask2].type(torch.LongTensor), reduction="sum"
        )
    ...
    def gif_approxi(self, res_tuple):
        ...
        h_estimate = tuple(
            grad1 - grad2 for grad1, grad2 in zip(res_tuple[1], res_tuple[2])
        )
        for _ in range(iteration):
            ...
            with torch.no_grad():
                h_estimate = [
                    v1 + (1 - damp) * h_estimate1 - hv1 / scale
