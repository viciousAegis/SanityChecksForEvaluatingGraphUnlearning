...
def run_experiment(percent_to_be_removed):
    ...
    acc_del_mask, f1_del_mask = test_new(model, temp_data)
    print("Accuracy and f1 score on the dataset with removed nodes: ", acc_del_mask, f1_del_mask)

    inference_results = (acc_del_mask, f1_del_mask)
    ...
    return original_results, inference_results
...
if __name__ == "__main__":
    ...
    for percent in percentages:
        ...
        original_results, inference_results = run_experiment(percent)
        result_row = (
            percent, 
            original_results[0], original_results[1],
            inference_results[0], inference_results[1],
        )
        ...
        with open('Citeseer_inferencerandomremoval_results.csv', 'w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(header)
            writer.writerows(results)
        
        print("Results saved to 'Citeseer_inferencerandomremoval_results.csv'")
