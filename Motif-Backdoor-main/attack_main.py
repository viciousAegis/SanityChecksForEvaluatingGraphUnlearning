...
def test_attack(clean_model, poi_model, device, train_graphs, test_graphs, poi_train_graphs, poi_test_graphs,args.target):
    # clean_model.eval()
    # poi_model.eval()
    ...
    #     if (labels[i] == pred[i]) and (labels[i] != args.target) :
    #         train_tatal += 1
    #         if poi_pred[i] == args.target:
    #             train_suc += 1
...
def main():
    ...
    target_neuron_path = Find_neuron_path(graphs, clean_model, args.target, train_trigger_num) if position_type_value == 'Neuron_Path' else 0
    ...
    if train_motif_name != 'non_trigger':
        if position_type_value == 'motif_subgraph':
            best_sub, poison_idx = motif_subgraph(args, clean_train_graphs, train_poi_rate, args.target,
                                          train_trigger_num, clean_model, device,max_nodes, train=True)
            train_graphs, _ = motif_poison(clean_train_graphs, train_poi_rate, args.target, train_motif_name, train_trigger_num, clean_model,target_neuron_path,
            ...
            train_graphs, attack_test_graphs, poi_list = GTA_B(args, clean_train_graphs, test_graphs, train_poi_rate, args.target,
                                          train_trigger_num, su_model, device,max_nodes, train=True)

        else:
            train_graphs, poi_list = motif_poison(clean_train_graphs, train_poi_rate, args.target, train_motif_name,
    ...
    if test_motif_name == 'non_trigger':
        ...
        if position_type_value == 'motif_subgraph':
            # 查看训练过程中的攻击中毒效果
            # attack_train_graphs, _ = motif_poison(clean_train_graphs, 0.05, args.target, test_motif_name,
            ...
            attack_test_graphs, _ = motif_poison(test_graphs, 0.05, args.target, test_motif_name, test_trigger_num,
            ...
            #查看训练过程中的攻击中毒效果
            # attack_train_graphs, _ = motif_poison(clean_train_graphs, 0.05, args.target,  test_motif_name, test_trigger_num,clean_model,target_neuron_path,
            #                                    graph_emb_target, graph_emb_other, train=False,position_type=position_type_value, ER_sub=G_gen)
            attack_test_graphs, _ = motif_poison(test_graphs, 0.05, args.target, test_motif_name, test_trigger_num,clean_model,target_neuron_path,
                                              graph_emb_target, graph_emb_other, train=False, position_type=position_type_value, ER_sub=G_gen)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # test attack
        train_asr, test_ars, poison_AMC, clean_acc_test = test_attack(clean_model, model, device,clean_train_graphs,test_graphs, clean_train_graphs, attack_test_graphs,args.target)
        ...
        print('dataset',args.dataset, 'args.target', args.target, 'position_type', position_type_value)
